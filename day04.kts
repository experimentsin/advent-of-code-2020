...
object ByrSpec : FieldSpec("byr", """(\d\d\d\d)""") {
    override fun validateMatch(match: MatchResult.Destructured): Boolean {
        val (year) = match
        return year.toInt() in 1920..2002
     }
}
...
object IyrSpec : FieldSpec("iyr", """(\d\d\d\d)""") {
    override fun validateMatch(match: MatchResult.Destructured): Boolean {
        val (year) = match
        return year.toInt() in 2010..2020
     }
}
...
object EyrSpec : FieldSpec("eyr", """(\d\d\d\d)""") {
    override fun validateMatch(match: MatchResult.Destructured): Boolean {
        val (year) = match
        return year.toInt() in 2020..2030
     }
}
...
object HgtSpec : FieldSpec("hgt", """(\d+)(.+)""") {
    override fun validateMatch(match: MatchResult.Destructured): Boolean {
        ...
        when (unit) {
            "cm" -> return iheight in 150..193
            "in" -> return iheight in 59..76
            else -> return false
        }
    }
}
...
object PidSpec : FieldSpec("pid", """(\d\d\d\d\d\d\d\d\d)""") {
    override fun validateMatch(match: MatchResult.Destructured): Boolean = true
}
...
fun isValidPassportPart2(pp: Passport): Boolean {
    for (field in requiredFieldsPart2) {
        val fieldData = pp.fields[field.name]
        if (fieldData == null) {
            // println("Missing $field")
            return false
        }
        if (!field.validate(fieldData)) {
...
fun processPart1() {
    ...
    val valid = passports.count(::isValidPassportPart1)
    println("Valid $valid")
}
...
fun processPart2() {
    // println("Passports $passports")
    val valid = passports.count(::isValidPassportPart2)
    println("Valid $valid")
}
